...
import managementCenter.InfoMsg;
import sales.SalesInfoDAO;
import sales.SalesInfoDTO;
...
class CntSHandler extends Thread {
		...
		while(true){
			try{
				...
				if(infodto.getCommand()==InfoMsg.EXIT){
					...
					sendDTO.setId(infodto.getId());
					sendDTO.setName(infodto.getName());
					...
					String exitAccepted = "["+infodto.getId()+"]님이 PC 사용을 종료하여 로그아웃하셨습니다.";
					...
					JOptionPane.showMessageDialog(null,"["+infodto.getName()+"]"+infodto.getMessage(),"[메세지 도착]",JOptionPane.INFORMATION_MESSAGE);}
	...
	public void updateAccount(InfoDTO infodto) {
		...
		salesInfoDTO.setPerchaseID(infodto.getId());
