   ...
   public ClientMgtFrame() {
      ...
      // setUndecorated(true);//투명창 만들기
       ...
       for(MemberDTO memberDTO : userList) {//벡터에 memberDTO 추가후 model에 추가
          ...
          vector.addElement(memberDTO.getName());
          vector.addElement(memberDTO.getPhonenumber());
          vector.addElement(memberDTO.getEmail());
   ...
   public void actionPerformed(ActionEvent e) {
      if(modifyCntB == e.getSource()) {//****************************************
         ...
         if(tableCnt.getSelectedRow()!=-1) { //테이블 선택안하고 버튼누를시 메세지 출력, 선택시 수정창 띄움
            ...
            memberDTO.setName((String) tableCnt.getValueAt(tableCnt.getSelectedRow(), 1));
            memberDTO.setPhonenumber((String) tableCnt.getValueAt(tableCnt.getSelectedRow(), 2));
            memberDTO.setEmail((String) tableCnt.getValueAt(tableCnt.getSelectedRow(), 3));
