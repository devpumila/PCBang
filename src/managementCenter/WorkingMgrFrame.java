...
import cash.CashMgrFrame;
import cash.CashierFrame;
import inventory.InvMntFrame;
...
public class WorkingMgrFrame extends JFrame implements ActionListener, ListSelectionListener {
	...
	public WorkingMgrFrame() {
		...
		monitorJP.setBounds(30, 80, 750, 600);// 780,620
		...
		seatJP[8] = new Seat9();
		seatJP[9] = new Seat10();
		seatJP[10] = new Seat11();
		seatJP[11] = new Seat12();
		for (int i = 0; i < 12; i++) {
			monitorJP.add(seatJP[i]);
		}
		con.add(monitorJP);
		...
		buttonJP1.add(memberMgtB);
		buttonJP1.add(inventoryB);
		buttonJP1.add(cashMgtB);
		buttonJP1.add(stopB);
		...
		clockJP.setBounds(840, 50, 280, 100);// 280,750
		con.add(clockJP);

		// 2-3 Type2 버튼 부착

		JPanel buttonJP2 = new JPanel(new GridLayout(1, 2, 10, 10));
		buttonJP2.setBounds(810, 390, 350, 100);// 760,750
		buttonJP2.add(addTimeB);
		buttonJP2.add(billingB);
		con.add(buttonJP2);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == memberMgtB) {
			...
			if (cashier == null)
				cashier = new CashierFrame(memberDTO);
			else {
				cashier.dispose();
				cashier = new CashierFrame(memberDTO);
			}
			...
			if (list.getFirstVisibleIndex() >= 0) {
				JOptionPane.showMessageDialog(this, "[경고]남아 있는 사용자가 있는 경우에는 종료할 수 없습니다.", "[프로그램 종료 오류]",
	...
	public static void main(String[] args) {
		...
		WorkingMgrFrame mgf = new WorkingMgrFrame();
	}
