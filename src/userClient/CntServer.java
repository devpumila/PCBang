...
public class CntServer extends JPanel implements Runnable {
	...
	private String serverIP = "127.0.0.1";// 각 pc마다 바뀔 서버 IP
	...
	public static CntServer getlogin(MemberDTO memberDTO) {
		CntServer monitor = new CntServer(memberDTO);
		monitorList.add(monitor);
		new InnerClientFrame(memberDTO);// 로그인화면 오픈
		return monitor;
	}// 생성 제약자

	public static CntServer getInstance(MemberDTO dto) {
		CntServer cnt = null;
		for (CntServer user : monitorList) {
			if (user.id.equals(dto.getId()))
				cnt = user;
		}
		return cnt;
	}// 'THE' CntServer 호출
		...
		while (true) {
			try {
				...
				// 일반 메세지
					...
					else if (dto.getCommand() == InfoMsg.DELIVERED) {// 메세지 수신시 재고 반영-물리적으로 메세지가 도달하면 물품이 제공된 것으로 간주하고,
																		// 재고에 반영한다.
						int itemNum = dto.getItemNum();
						int itemMnt = dto.getItemMnt();
						InventoryDAO dao = InventoryDAO.getInstance();
						dao.takeInventoryOrder(itemNum, itemMnt);
					}
				}
			} // 확인하면 DB 반영
			catch (IOException e) {
