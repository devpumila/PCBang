...
public class InnerClientFrame extends JFrame implements ActionListener, Runnable {
	...
	public InnerClientFrame(MemberDTO memberDTO) {
		...
		JLabel nameL2 = new JLabel(memberDTO.getName(), SwingConstants.CENTER);
		nameL1.setFont(nameLF);
		nameL2.setFont(nameLF);
		...
		uTimeJP.add(uTimeL1);
		uTimeJP.add(uTimeL2);
		uTimeJP.add(prepayMntL1);
		uTimeJP.add(prepayMntL2);
		uTimeJP.add(nameL1);
		uTimeJP.add(nameL2);
		con.add(uTimeJP);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == messageB) {
			if (chat == null)
				new ClientChat(memberDTO);
			else {
				chat.dispose();
				chat = new ClientChat(memberDTO);
			}
		}
		if (e.getSource() == orderB) {
			if (order == null)
				order = new OrderMenu(memberDTO);
			else {
				order.dispose();
				order = new OrderMenu(memberDTO);
			}
		...
		if (e.getSource() == stopB) { // 유열씨 메소드
			if (stopSW == 0) {// stopSW기본값0, 처음 눌렸을 때 작동
				...
				stopB.setIcon(new ImageIcon("img/chains.png"));
		...
		if (e.getSource() == exitB) {// 화종씨 구현
			...
			if (result == JOptionPane.YES_OPTION) {
				...
				infoDTO.setName(userName);
				infoDTO.setId(userID);
