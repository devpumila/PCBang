...
import managementCenter.InfoDTO;
...
public class OrderMenu extends JFrame implements ActionListener, ListSelectionListener {
	...
	private JPanel labelJP, itemInfoTJP, mntSelJP, otherSelJP, buttonJP, messageTJP, totCostTJP;
	private int stock;
	...
	public OrderMenu(MemberDTO memberDTO) {
		...
		labelJP.add(itemNameL);
		labelJP.add(itemuCostL);
		labelJP.add(mntselL);
		labelJP.add(totCostL);
		...
		itemInfoTJP.add(itemNameF);
		itemInfoTJP.add(itemuCostF);
		con.add(itemInfoTJP);
	...
	public void actionPerformed(ActionEvent e) {
		if (otherRB.isSelected()) {
			...
			if (e.getSource() == otherMntF && !otherMntF.getText().equals(""))
	...
	public void pushOrderInfo() {
		...
		if (oneRB.isSelected())
			...
			dto.setItemMnt(Integer.parseInt(otherMntF.getText()));

		// 재고이상 주문시
		if (dto.getItemMnt() > stock) {
			JOptionPane.showMessageDialog(this, "재고 이상의 물품은 주문하실 수 없습니다. 현재 남은 수량: " + stock + "(개)", "[알림]",
	...
	public String getTotal() {// 선택한 물품에 대한 전체 비용 출력
		int totalCost = 0;
		if (list.isSelectionEmpty() || stock == 0 || itemuCostF.getText().equals(""))
			return "";// 선택한 것이 없음,재고없는 상태 선택, 기타를 선택하고도 아무런 입력이 없을 때.
		else {
			if (oneRB.isSelected())
				totalCost = Integer.parseInt(itemuCostF.getText());
			else if (twoRB.isSelected())
				totalCost = Integer.parseInt(itemuCostF.getText()) * 2;
			else if (threeRB.isSelected())
				totalCost = Integer.parseInt(itemuCostF.getText()) * 3;
			else if (otherRB.isSelected() && !otherMntF.getText().equals(""))
				try {
					totalCost = Integer.parseInt(itemuCostF.getText()) * Integer.parseInt(otherMntF.getText());
	...
	public void erase() {// 항목 초기화
		...
		twoRB.setSelected(false);
		threeRB.setSelected(false);
		otherRB.setSelected(false);
		otherMntF.setText("");
		otherMntF.setEditable(false);
		totCostF.setText("");
		messageF.setText("기타 문의사항을 입력해주세요.");
		messageF.setEditable(false);
		orderB.setEnabled(false);
		eraseB.setEnabled(false);
		messageF.setEditable(false);
