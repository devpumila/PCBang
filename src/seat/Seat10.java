...
public class Seat10 extends JButton implements Runnable {
	...
	public Seat10() {
		...
		name = new JLabel("[10번] 빈 좌석",SwingConstants.CENTER);
	...
	public void run(){
		while(true){
			if(dao.getOnOff(10)==0) {
				this.setBackground(new Color(11,42,66));
				name.setFont(vacantF);
				lb = new LineBorder(Color.BLUE,5);
				String text = "[10번] 빈 좌석";
				name.setText(text);
				this.setBorder(lb);
			}
			else if(dao.getOnOff(10)==1) {
				this.setBackground(new Color(72,121,57));
				name.setFont(normalF);
				String text1 ="[10]번   "+dao.getName(10)+" 번";
				String text2 ="잔여시간  :  "+(dao.getTime(10)/60)+" 시간"
						+(dao.getTime(10)%60)+"  분";
				name.setText(text1);
